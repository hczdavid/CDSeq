...
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score'){
  ...
  #save results
  #write.table(rbind(header,output), file="BRCA_mRNA_CIBERSORT-Results.txt", sep="\t", row.names=F, col.names=F, quote=F)
  ...
  if(!absolute){colnames(obj) <- c(colnames(X),"P-value","Correlation","RMSE")}else{
