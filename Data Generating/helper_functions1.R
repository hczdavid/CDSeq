...
#This code is adopted from https://github.com/favilaco/deconv_benchmark with some minor adjustments.
#source('./CIBERSORT.R')
...
Generator <- function(sce, phenoData, Num.mixtures = 100, pool.size = 100, min.percentage = 1, max.percentage = 99, seed = 24){ 
  ...
  for(y in 1:Num.mixtures){
    ...
    while(!exists("P")){
      ...
      P <- as.vector(gtools::rdirichlet(1, rep(5,num.CT.mixture)))
      P = round(P, digits = 2)  #sum to 1
    ...
    # add by David to calculate the counts propotion
    mysubT <- sce[,colnames(sce) %in% chosen_cells]
    rownames(phenoData) <- phenoData[,1]
    chosen_celltype <- phenoData[colnames(mysubT),2]
    colnames(mysubT) <- chosen_celltype
    mysubT1 <- mergecol(mysubT)
    mysubP <- (colSums(mysubT1)/sum(colSums(mysubT1))) %>% round(2)
    newP <- P
    rownames(newP) <- newP$CT
    newP[names(mysubP),2] <- mysubP
    
    
    P$mix = paste("mix",y,sep="")
    newP$mix = paste("mix", y, sep = "")
    
    Tissues[[y]] <- T
    Proportions[[y]] <- P
    nProportions[[y]] <- newP
    
    rm(list=c("T","P","chosen_cells","missing.CT", "newP"))
    
  }
  
  P = do.call(rbind.data.frame, Proportions)
  T = do.call(cbind.data.frame, Tissues)
  newP = do.call(rbind.data.frame, nProportions)
  ...
  newP = data.table::dcast(newP, CT ~ mix, 
                        value.var = "expected",
                        fun.aggregate = sum) %>% data.frame(.,row.names = 1) 
  
  newP = newP[,gtools::mixedsort(colnames(newP))]

  return(list(T = T, P = P, newP = newP))
...
Deconvolution <- function(T, C, method, phenoDataC, P = NULL, elem = NULL, STRING = NULL,  refProfiles.var){ #marker_distrib,
    ...
    #     
    #     require(xbioc)
    #     C.eset <- Biobase::ExpressionSet(assayData = as.matrix(train_cellID),phenoData = Biobase::AnnotatedDataFrame(phenoDataC))
    ...
    if(method=="CIBERSORT"){ #without QN. By default, CIBERSORT performed QN (only) on the mixture.
        ...
        common_gene <- intersect(rownames(C), rownames(refProfiles.var))
        C_EPIC[["sigGenes"]] <- rownames(C[common_gene,common_CTs])
        C_EPIC[["refProfiles"]] <- as.matrix(C[common_gene,common_CTs])
        C_EPIC[["refProfiles.var"]] <- refProfiles.var[common_gene,common_CTs]
        ...
        RESULTS = CellMix::ged(as.matrix(T), ncol(C), method = "DSA")@fit@H
        RESULTS = apply(RESULTS,2,function(x) ifelse(x < 0, 0, x)) #explicit non-negativity constraint
        RESULTS = apply(RESULTS,2,function(x) x/sum(x)) #explicit STO constraint
        ...
        RESULTS <- CellMix::ged(as.matrix(T), ncol(C), method = "ssKL", sscale = FALSE, maxIter=500, log = FALSE)@fit@H 
        rownames(RESULTS) <- colnames(C)
        ...
        RESULTS <- CellMix::ged(object = as.matrix(T), x = ncol(C),method = "ssFrobenius", maxIter = 500)@fit@H #equivalent to coef(CellMix::ged(T,...)

    } else if(method=="dtangle"){#Only works if T & C are log-transformed

        require(dtangle)
        mixture_samples = t(T[rownames(C),])
        ...
        RESULTS = t(dtangle::dtangle(Y=mixture_samples, reference=reference_samples)$estimates)
...
mergecol <- function(m){
  coln <- table(colnames(m))
  none1name <- names(coln[coln!=1])
  m1 <- m[,names(coln[coln==1]),drop=F]
  for (i in 1:length(none1name)) {
    myg <- as.matrix(rowSums(m[,which(colnames(m)==none1name[i])]))
    colnames(myg) <- none1name[i]
    m1 <- cbind(m1,myg)
  }
  return(m1)
}
